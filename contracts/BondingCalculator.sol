...
contract BondingCalculator is IBondingCalculator {
    ...
    IERC20 public immutable sFORCE;

    constructor( address _sFORCE ) {
        require( _sFORCE != address(0) );
        sFORCE = IERC20(_sFORCE);
    }
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(sFORCE) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(sFORCE), "not a Time lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** sFORCE.decimals() ) ).div( getTotalValue( _pair ) );
    }
}